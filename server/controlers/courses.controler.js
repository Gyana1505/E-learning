...
export const fetchLectures=TryCatch(async(req,res)=>{
    ...
    return res.status(400).json({
        message:"You have not subscribe to this course"
    })
    res.json({
        lectures
    })
...
export const addprogess = TryCatch(async(req,res)=>{
...
export const getYourProgress = TryCatch(async (req, res) => {
    ...
    if (!progress || progress.length === 0) {
        return res.status(200).json({
            ...
            completedLectures: 0,
            allLectures: await Lecture.countDocuments({ course: req.query.course }),
            progress: [],
        });
    }
